...
export function QuoteBuilder({
  ...
  const sep = isIOS ? "&" : "?";
  const priceTerms = introductoryDiscount
    ? `$${firstMonth} for the first month with ${Math.round(introductoryDiscount * 100)}% off, then $${price}/mo`
    : `$${price}/mo`;
  const msgBody = `Hey, I'm interested in the ${SCOPE[scope]?.text || "basic"} ${FREQ[frequency]?.text || "monthly"} plan (${priceTerms}). Can you confirm my quote and availability?`;
  const smsHref = `sms:9168472095${sep}body=${encodeURIComponent(msgBody)}`;

  const handleFreq = (i: number) => {
    setFrequency(i);
    setInteracted(true);
  };
  const handleScope = (i: number) => {
    setScope(i);
    setInteracted(true);
  };

  const btnLabel = introductoryDiscount
    ? "Text us to claim 10% off"
    : interacted
  ...
  return (
    <div
      className={`rounded-3xl overflow-hidden w-full ${compact ? "max-w-[440px]" : "max-w-[540px]"} mx-auto shadow-[0_2px_40px_rgba(0,0,0,0.12)] border ${cardBg}`}
    >
      <div
      ...
      >
        ...
        <div className={compact ? "text-[#1a1a1a]" : "text-[#1a1a1a]"}>
          ...
          >
            ${introductoryDiscount ? firstMonth : price}
          </span>
          <span
            className={`${compact ? "text-[20px]" : "text-[22px]"} font-normal text-[#8e8e93]`}
          >
      ...
      <div className={`px-6 ${compact ? "pb-5 space-y-4" : "pb-8 space-y-7"}`}>
        <div>
          <p
            className={`text-[12px] font-semibold text-[#8e8e93] uppercase tracking-wider ${compact ? "mb-2" : "mb-3"}`}
          >
            How often
          </p>
          <div
            className={`flex ${compact ? "bg-black/[0.06]" : "bg-[#f2f2f7]"} rounded-[10px] p-1`}
          >
            {FREQ.map((f, i) => (
              <button
                key={i}
                aria-pressed={frequency === i}
        ...
        <div>
          ...
          {compact ? (
            <div className="grid grid-cols-2 gap-x-3 gap-y-1.5">
              {ALL_SERVICES.filter((svc) => svc.minScope <= scope).map(
                (svc, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={`text-[13px] leading-tight ${included ? "text-[#1a1a1a]" : "text-[#c7c7cc]"}`}
                      >
                        {svc.name}
                      </span>
                    </div>
              ...
              {ALL_SERVICES.map((svc, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`text-[15px] ${included ? "text-[#1a1a1a]" : "text-[#c7c7cc]"}`}
                    >
                      {svc.name}
                    </span>
                  </div>
